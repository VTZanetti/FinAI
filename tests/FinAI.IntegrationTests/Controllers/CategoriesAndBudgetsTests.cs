...
public class CategoriesAndBudgetsTests : IClassFixture<FinAiTestFixture>
{
    ...
    public async Task UpdateSystemCategory_ReturnsForbidden()
    {
        ...
        var categories = await (await _client.GetAsync("/api/v1/categories")).ReadAsync<List<CategoryResponse>>();
        var system = categories!.First(c => c.IsSystem);

        // Act
        var response = await _client.PutAsJsonAsync($"/api/v1/categories/{system.Id}", new { name = "Hackeada" });
    ...
    public async Task DeleteSystemCategory_ReturnsForbidden()
    {
        ...
        // Act
        var response = await _client.DeleteAsync($"/api/v1/categories/{system.Id}");
    ...
    {
        // Arrange
        var create = await _client.PostAsJsonAsync("/api/v1/categories", new { name = "Pets", subcategory = "Vet" });
        create.StatusCode.Should().Be(HttpStatusCode.Created);
        var category = await create.ReadAsync<CategoryResponse>();

        // Act
        var delete = await _client.DeleteAsync($"/api/v1/categories/{category!.Id}");
    ...
    {
        // Arrange
        await _client.PostAsJsonAsync("/api/v1/categories", new { name = "Duplicada" });

        // Act
        var response = await _client.PostAsJsonAsync("/api/v1/categories", new { name = "Duplicada" });
    ...
    public async Task CreateBudget_ShowsProgress()
    {
        ...
        var accountResponse = await _client.PostAsJsonAsync("/api/v1/accounts", new
        {
            name = "Conta Budget",
            type = "Checking",
            currency = "BRL",
    ...
    public async Task CreateBudget_DuplicatePeriod_ReturnsConflict()
    {
        ...
        var first = await _client.PostAsJsonAsync("/api/v1/budgets", payload);
        first.StatusCode.Should().Be(HttpStatusCode.Created);

        // Act
        var second = await _client.PostAsJsonAsync("/api/v1/budgets", payload);
    ...
    public async Task UpdateBudget_ChangesLimit()
    {
        ...
        var categories = await (await _client.GetAsync("/api/v1/categories")).ReadAsync<List<CategoryResponse>>();
        var other = categories!.First(c => c.Name == "Other");
        var create = await _client.PostAsJsonAsync("/api/v1/budgets", new
        {
            categoryId = other.Id,
            month = 9,
            year = 2026,
        ...
        // Act
        var response = await _client.PutAsJsonAsync($"/api/v1/budgets/{budget!.Id}", new { limitAmount = 750m });
